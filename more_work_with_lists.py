...
# ALL
# list_with_data = [22, 0, 0, 0]
# all_true = all(list_with_data)
#
# at_least_one = any(list_with_data)
#
# maximum = max(some_list)
# mimimum = min(some_list)
# summa = sum(some_list, start=10)

some_stfr_list = ["zlkfsvjz", ";lijlisfvdf;nlk", "lkj", []]
max_str = max(some_stfr_list, key=len)
min_str = min(some_stfr_list, key=len)
